...
input_loop:
    beq $t2, $t0, find_max     # Si ya ingresamos todos los números, ir a buscar el mayor
...
find_max:
    lw $t3, numbers            # Cargar el primer número en $t3 (asumido como el mayor)
    li $t2, 1                  # Inicializar índice en 1 (empezamos desde el segundo número)

max_loop:
    beq $t2, $t0, print_result # Si ya revisamos todos los números, ir a imprimir el mayor

    sll $t4, $t2, 2            # Calcular el offset del siguiente número
    lw $t5, numbers($t4)       # Cargar el siguiente número

    ble $t3, $t5, update_max   # Si el número actual es mayor que el guardado, actualizar el máximo
    j next_num

update_max:
    move $t3, $t5              # Actualizar el valor máximo a $t3

next_num:
    addi $t2, $t2, 1           # Incrementar el índice
    j max_loop                 # Repetir el ciclo
...
print_result:
    ...
    move $a0, $t3              # Pasar el valor máximo encontrado
